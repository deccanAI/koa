...
module.exports = {
  ...
  set body (val) {
    ...
    if (val == null) {
      if (!statuses.empty[this.status]) {
        // Special handling for JSON responses with null body
      ...
      // Mark explicitly set null bodies
      if (val === null) this._explicitNullBody = true
      // Remove headers that don't make sense with empty body
    ...
    // Check if we need to set content type
    ...
    // Handle stream bodies
    if (val instanceof Stream) {
      // Clean up stream when response is finished
      onFinish(this.res, destroy.bind(null, val))
      
      // Only set up error handler if this is a new stream
      if (original !== val) {
        // Forward stream errors to context error handler
        val.once('error', err => {
          // Only call onerror if the context exists and has an onerror method
          if (this.ctx && typeof this.ctx.onerror === 'function') {
            this.ctx.onerror(err)
          }
        })
        
        // When overwriting an existing stream, remove Content-Length
    ...
    // Handle Web API Response
    if (typeof Response !== 'undefined' && val instanceof Response) {
      this.status = val.status
      if (setType) this.type = 'bin'
      
      // Copy all headers from the Response object
  ...
  get writable () {
    // First check if response is already finished
    ...
    if (this.headerSent) {
      // If headers were sent but the response isn't finished,
      // we can still write the body
      const socket = this.res.socket
      // No socket means we can't write, but the response might still be writable
      if (!socket) return this.res.writable !== false
      // Check if the underlying socket is writable
      return socket.writable
    }
    
    // If no headers were sent, we can still write the response
    return true
  },
